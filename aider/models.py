...
def load_model_settings():
    """Load all model settings from YAML files in the models directory."""
    model_settings = []
    ...
    for yml_file in models_dir.rglob("*.yml"):
        try:
            with open(yml_file) as f:
                ...
                # Handle settings with 'models:' key
                if isinstance(settings, dict) and 'models' in settings:
                    settings = settings['models']
                
                # Handle list of models
                if isinstance(settings, list):
                    for setting in settings:
                        if setting.get('prompt'):  # If prompt exists and is non-empty
                            setting['name'] = f"{setting.get('provider', '')}/{setting['name']}".strip('/')
                        model_settings.append(ModelSettings(**setting))
                else:
                    if settings.get('prompt'):  # If prompt exists and is non-empty
                        settings['name'] = f"{settings.get('provider', '')}/{settings['name']}".strip('/')
                    model_settings.append(ModelSettings(**settings))
