...
class TestFileSummary:
    ...
    def test_summarize_chunk(self, mock_io, mock_model, mock_cache, test_file):
        summary = FileSummary(mock_io, mock_cache, mock_model, test_file)
        content = Path(test_file).read_text()
        
        # Mock the send_completion function
        with patch('aider.file_summary.send_completion') as mock_send:
            mock_completion = MagicMock()
            mock_completion.usage.prompt_tokens = 100
            mock_completion.usage.completion_tokens = 50
            mock_send.return_value = (mock_completion, [])
            
            result, prompt_tokens, completion_tokens, cost = summary.summarize_chunk(content)
            
            assert prompt_tokens == 100
            assert completion_tokens == 50
            assert cost == (100 * 0.001) + (50 * 0.002)  # Based on mock model costs
